...
st.sidebar.title("Doctor Steve's project carried out by Yazid")
logo_path = "idea.png"  # Remplacez par le chemin de votre image
st.image(logo_path, caption="", width=70) 
...
def main():
    st.title('Images Based on CBIR')
    uploaded_file = st.file_uploader('Upload an image', type=['jpg', 'jpeg', 'png'])

    descriptor = st.selectbox('Select Descriptor', ['glcm', 'bitdesc'])

    distance_metric = st.selectbox('Select Distance Metric', ['manhattan', 'euclidean', 'chebyshev', 'canberra'])
...
st.markdown("<h5>Copyright © 2024 Yazid Titikpina. Tous droits réservés.</h5>", unsafe_allow_html=True)
